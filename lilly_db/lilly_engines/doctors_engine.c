...
static boolean LILLY_CALL_CONVENTION build_indexes(const p_lilly_db_engine self,const struct t_lilly_db_handle *db_handle)
{
  ...
  uint64 offset;
	 ...
	 while (TRUE)
	 {
	   if (!lilly_db_add_index(db_handle,iterator->code_medecin.unique_index,lilly_db_get_current_record_offset(db_handle))) break;
...
static struct t_lilly_db_handle* LILLY_CALL_CONVENTION new_database(const p_lilly_db_engine self,const puchar database_path,const unsigned_int db_path_len,const uint64 databse_flags,const uint32 operator_id,const uint32 session_id)
{
  struct t_lilly_db_handle* db_handle;
	 ...
	 db_handle = lilly_db_create_db_handle(database_path,db_path_len,MEDECIN_RECORD_SIZE,LILLY_DB_DATABASE_HEADER_SIZE,operator_id,session_id,DOCTOR_DB_SIGNATURE,
...
static struct t_lilly_db_handle* LILLY_CALL_CONVENTION load_database(const p_lilly_db_engine self,const puchar database_path,const unsigned_int db_path_len,const uint32 operator_id,const uint32 session_id)
{
  struct t_lilly_db_handle* db_handle;
	 ...
	 db_handle = lilly_db_open_db_handle(database_path,db_path_len,DOCTOR_DB_SIGNATURE,operator_id,session_id,&self->error_code);
...
{
  p_medecin_record record = (p_medecin_record)record_ptr;
  p_medecin_record iterator;


	 self->error_code = ERROR_SUCCESS;

	 record->code_medecin.unique_index = lilly_db_get_unique_autoinc_key(db_handle);

	 if (record->code_medecin.unique_index == IO_ERROR)
	 {
	   self->error_code = lilly_db_get_last_error(db_handle);
	   return FALSE;
	 }

	 record->code_medecin.crc32 = self->get_record_crc(self,db_handle,record);

     if (!record->code_medecin.crc32) return FALSE;
	 ...
	 if (!lilly_db_append_record(db_handle,(puchar)record,MEDECIN_RECORD_SIZE))
	 {
	   self->error_code = lilly_db_get_last_error(db_handle);
	   return FALSE;
	 }

     if (!lilly_db_add_index(db_handle,record->code_medecin.unique_index,lilly_db_get_current_record_offset(db_handle)))
	 {
	   self->error_code = lilly_db_get_last_error(db_handle);
...
static boolean LILLY_CALL_CONVENTION edit_record(const p_lilly_db_engine self,const struct t_lilly_db_handle *db_handle,const opaque_ptr record_ptr,const uint64 record_offset)
{
  ...
  p_medecin_record iterator;
	 ...
	 if (lilly_db_edit_record(db_handle,(puchar)record,MEDECIN_RECORD_SIZE,record_offset)) return TRUE;
...
static boolean LILLY_CALL_CONVENTION delete_record(const p_lilly_db_engine self,const struct t_lilly_db_handle *db_handle,const uint64 record_offset)
{
  ...
  uint32 rec_id;
	 ...
	 record = (p_medecin_record)lilly_db_read_record(db_handle,MEDECIN_RECORD_SIZE,record_offset);
	 ...
	 rec_id = record->code_medecin.unique_index;
...
static opaque_ptr LILLY_CALL_CONVENTION code_to_record(const p_lilly_db_engine self,const struct t_lilly_db_handle *db_handle,const uint32 rec_code)
{
  p_medecin_record iterator;
	 ...
	 iterator = lilly_db_index_to_record(db_handle,MEDECIN_RECORD_SIZE,rec_code);
	 ...
	 while (TRUE)
	 {
	   if (iterator->code_medecin.unique_index == rec_code) break;
...
p_lilly_db_engine LILLY_CALL_CONVENTION get_doctors_db_engine(const opaque_ptr session_manager,const punsigned_int error_code)
{
...
void LILLY_CALL_CONVENTION release_doctors_db_engine(const p_lilly_db_engine db_engine)
{
