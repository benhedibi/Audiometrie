...
typedef struct {
				 uint64 indexes[MAXWORD];
			   } t_lilly_db_index_block,*p_lilly_db_index_block;

typedef struct {
				 unsigned_int index_blocks_count;
				 unsigned_int io_handle;
				 p_lilly_db_index_block blocks;
			   } t_lilly_db_index_block_params;
...
typedef struct t_lilly_db_handle {
                                   ...
                                   t_lilly_db_index_block_params index_blocks_params;
...
static boolean set_record_offset(const p_lilly_db_handle db_handle,const uint64 record_offset)
{
	 ...
	 if (!lilly_io_strm_set_current_stream_offset(db_handle->io_stream_handle,record_offset,FALSE))
	 {
       db_handle->error_code = lilly_io_strm_get_last_error(db_handle->io_stream_handle);
	   return FALSE;
     }
...
static puchar lilly_db_get_previous_record_ex(const p_lilly_db_handle db_handle)
{
  puchar record;
	 ...
	 if ((int64)db_handle->db_cursor.current_record_offset < db_handle->db_cursor.first_record_offset)
	 {
	   db_handle->db_cursor.current_record_offset = db_handle->db_cursor.first_record_offset;
	   return NULL;
     }
...
static boolean lilly_db_get_index_db_file_path(const p_lilly_db_handle db_handle,const puchar index_db_file_path,const unsigned_int max_db_file_path_len)
{

#define IDX_EXTENSION 0x007800640069002E // .idx


	 if (db_handle->db_path_len + 10 > max_db_file_path_len)
	 {
	   db_handle->error_code = IO_ERROR;
	   return FALSE;
	 }

	 *(puint16)&index_db_file_path[db_handle->db_path_len + 8] = 0; // warm up the cache

	 avcore_memcpy(index_db_file_path,&db_handle->db_path,db_handle->db_path_len);

	 *(puint64)&index_db_file_path[db_handle->db_path_len] = IDX_EXTENSION;
...
static void lilly_db_delete_index_db_file(const p_lilly_db_handle db_handle,const puchar index_db_file_path)
{

	 if (db_handle->index_blocks_params.io_handle)
	 {
	   lilly_io_close_file(db_handle->index_blocks_params.io_handle);
	   db_handle->index_blocks_params.io_handle = 0;
	 }

	 if ((db_handle->index_blocks_params.index_blocks_count) && (db_handle->index_blocks_params.blocks))
	 {
	   lilly_free(db_handle->index_blocks_params.blocks,db_handle->index_blocks_params.index_blocks_count * sizeof(*db_handle->index_blocks_params.blocks));
	 }

	 db_handle->index_blocks_params.index_blocks_count = 0;
	 db_handle->index_blocks_params.blocks = NULL;

	 if (index_db_file_path)
	 {
	   lilly_io_delete_file(index_db_file_path);
	 }

     return;
}

static boolean lilly_db_create_db_index_file(const p_lilly_db_handle db_handle,const puchar db_index_file_path)
{
  unsigned_int mem_size;


	 db_handle->error_code = ERROR_SUCCESS;

	 if (db_handle->db_header.last_key_index > sizeof(*db_handle->index_blocks_params.blocks))
	 {
	   db_handle->index_blocks_params.index_blocks_count = db_handle->db_header.last_key_index / sizeof(*db_handle->index_blocks_params.blocks);
	 }
	 else db_handle->index_blocks_params.index_blocks_count = 1;

	 mem_size = db_handle->index_blocks_params.index_blocks_count * sizeof(*db_handle->index_blocks_params.blocks);

	 db_handle->index_blocks_params.blocks = (p_lilly_db_index_block)lilly_malloc(mem_size);

	 if (!db_handle->index_blocks_params.blocks)
	 {
	   db_handle->error_code = MEM_ALLOC_ERROR;
	   return FALSE;
	 }

	 avcore_memset(db_handle->index_blocks_params.blocks,0,mem_size);

	 db_handle->index_blocks_params.io_handle = lilly_io_create_file(db_index_file_path);

	 if ((signed_int)db_handle->index_blocks_params.io_handle <= 0)
	 {
	   db_handle->error_code = IO_ERROR;
       return FALSE;
	 }

	 if (!lilly_io_file_write(db_handle->index_blocks_params.io_handle,(puchar)db_handle->index_blocks_params.blocks,mem_size))
	 {
	   db_handle->error_code = IO_ERROR;
	   return FALSE;
	 }

	 db_handle->error_code = LILLY_DB_VIRGIN_INDEX_FILE;

	 return TRUE;
}

static boolean lilly_db_open_indexes_db(const p_lilly_db_handle db_handle)
{
  uint64 db_index_file_size;
  unsigned_int mem_size;
  uchar db_index_file[260 * 2];


	 db_handle->error_code = ERROR_SUCCESS;

     if (!lilly_db_get_index_db_file_path(db_handle,&db_index_file,sizeof(db_index_file))) return FALSE;

	 db_handle->index_blocks_params.io_handle = lilly_io_open_file(&db_index_file,lilly_io_fmOpen,lilly_io_faReadWrite);
	 ...
	 db_index_file_size = lilly_io_get_file_size_ex(db_handle->index_blocks_params.io_handle);

	 if ((signed_int)db_index_file_size < sizeof(*db_handle->index_blocks_params.blocks))
	 {
	   db_handle->error_code = IO_ERROR;
	   return FALSE;
	 }

	 if (db_index_file_size % sizeof(*db_handle->index_blocks_params.blocks) != 0)
	 {
	   db_handle->error_code = LILLY_DB_CORRUPTED_DATABASE_ERROR;
       return FALSE;
	 }

	 db_handle->index_blocks_params.index_blocks_count = (unsigned_int)(db_index_file_size / sizeof(*db_handle->index_blocks_params.blocks));

	 db_handle->index_blocks_params.blocks = lilly_malloc((unsigned_int)db_index_file_size);
	 ...
	 if (!lilly_io_file_read(db_handle->index_blocks_params.io_handle,(puchar)db_handle->index_blocks_params.blocks,(unsigned_int)db_index_file_size))
	 {
	   db_handle->error_code = IO_ERROR;
	   return FALSE;
	 }
...
p_lilly_db_handle lilly_db_create_db_handle(const puchar db_path,const unsigned_int db_path_len,const uint64 record_size,const uint64 header_size,const uint32 operator_id,const uint32 session_id,const uint64 db_signature,const uint64 db_flags,const punsigned_int error_code)
{
  ...
  uint32 encryption_key;
	 ...
	 *error_code = ERROR_SUCCESS;
	 ...
	 if (CHECK_ATTRIBUTE(db_flags,LILLY_DB_INDEXING_FLAG))
	 {
	   uchar index_db_file_path[260 * 2];

	   if (!lilly_db_get_index_db_file_path(handle,&index_db_file_path,sizeof(index_db_file_path)))
	   {
		 lilly_io_strm_close_stream(handle->io_stream_handle);
		 lilly_free(handle,sizeof(*handle));
		 return NULL;
	   ...
	   if (lilly_io_file_exists(&index_db_file_path))
	   {
		 lilly_io_delete_file(&index_db_file_path);
	   }

	   if (!lilly_db_create_db_index_file(handle,&index_db_file_path))
	   {
		 lilly_io_strm_close_stream(handle->io_stream_handle);
		 lilly_free(handle,sizeof(*handle));
		 return NULL;
...
p_lilly_db_handle lilly_db_open_db_handle(const puchar db_path,const unsigned_int db_path_len,const uint64 db_signature,const uint32 operator_id,const uint32 session_id,const punsigned_int error_code)
{
  ...
  uint32 encryption_key;
	 ...
	 *error_code = ERROR_SUCCESS;
	 ...
	 handle->io_stream_handle = lilly_io_strm_open_stream(db_path,(unsigned_int)handle->db_header.record_size,error_code,encryption_key,(unsigned_int)handle->db_header.database_size,
     ...
     if (CHECK_ATTRIBUTE(handle->db_header.flags,LILLY_DB_INDEXING_FLAG))
	 {
	   uchar index_db_file_path[260 * 2];

	   if (!lilly_db_get_index_db_file_path(handle,&index_db_file_path,sizeof(index_db_file_path))) goto failure;

	   if ((handle->db_header.record_count / sizeof(*handle->index_blocks_params.blocks) > handle->index_blocks_params.index_blocks_count) ||
	   (!lilly_io_file_exists(&index_db_file_path)))
	   {
		 //lilly_io_delete_file(&index_db_file_path);

		 if (!lilly_db_create_db_index_file(handle,&index_db_file_path)) goto failure;
         *error_code = handle->error_code;
	   }
	   else
	   {
		 if (!lilly_db_open_indexes_db(handle)) goto failure;
	   }
	 }
...
void lilly_db_close_db_handle(const p_lilly_db_handle db_handle)
{
  boolean no_error = TRUE;
	 ...
	 if (db_handle->index_blocks_params.io_handle)
	 {
	   if ((db_handle->index_blocks_params.index_blocks_count) && (db_handle->index_blocks_params.blocks))
	   {
		 no_error = (((int64)lilly_io_file_seek(db_handle->index_blocks_params.io_handle,0,lilly_soFromBeginning) >= 0) &&
		 lilly_io_file_write(db_handle->index_blocks_params.io_handle,(puchar)db_handle->index_blocks_params.blocks,
		 db_handle->index_blocks_params.index_blocks_count * sizeof(*db_handle->index_blocks_params.blocks)));
	   }
	 ...
	 if (db_handle->io_stream_handle)
	 {
	   REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_CRASH_STATE_FLAG);
...
boolean lilly_db_purge_database(const p_lilly_db_handle db_handle)
{
  unsigned_int lazy_mode_state;
	 ...
	 if (db_handle->index_blocks_params.index_blocks_count)
	 {
	   lilly_free(db_handle->index_blocks_params.blocks,db_handle->index_blocks_params.index_blocks_count * sizeof(*db_handle->index_blocks_params.blocks));
	   ...
	   if (db_handle->index_blocks_params.io_handle)
	   {
		 ...
		 lilly_db_get_index_db_file_path(db_handle,&index_db_file_path,sizeof(index_db_file_path));
       }
	 }
	 ...
	 SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_CRASH_STATE_FLAG);
...
boolean lilly_db_write_record(const p_lilly_db_handle db_handle,const puchar data,const unsigned_int data_size,const uint64 record_offset)
{
  puchar record;
	 ...
	 return ((lilly_db_write_record_ex(db_handle,(p_lilly_db_record_header)record,record_offset)) && (lilly_db_write_header(db_handle,NULL,0)));
...
boolean lilly_db_edit_record(const p_lilly_db_handle db_handle,const puchar record,const unsigned_int record_size,const uint64 record_offset)
{
	 ...
	 return lilly_db_write_record(db_handle,record,record_size,record_offset);
}
...
boolean lilly_db_set_current_record_offset(const p_lilly_db_handle db_handle,const uint64 offset)
{
	 ...
	 db_handle->db_cursor.current_record_offset = offset;
...
boolean lilly_db_get_record_time_stamp_infos(const p_lilly_db_handle db_handle,const uint64 record_offset,const p_lilly_db_time_stamp_informations time_stamp_infos)
{
  puchar record;
	 ...
	 *time_stamp_infos = *(p_lilly_db_time_stamp_informations)(&((p_lilly_db_record_header)record)->time_stamp_infos);
...
boolean lilly_db_get_database_informations(const p_lilly_db_handle db_handle,const p_lilly_db_database_infos db_infos)
{
	...
	db_infos->indexing = (CHECK_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_INDEXING_FLAG));
	db_infos->crash_flag = (CHECK_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_CRASH_STATE_FLAG));

	return TRUE;
}

boolean lilly_db_set_database_informations(const p_lilly_db_handle db_handle,const p_lilly_db_database_infos db_infos)
{
	...
	if (db_infos->session_id)
	{
	  db_handle->db_header.session_id = db_infos->session_id;
	}

	if (db_infos->header_crc)
	{
	  db_handle->db_header.header_crc = db_infos->header_crc;
	}

	if (db_infos->encryption)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_ENCRYPTION_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_ENCRYPTION_FLAG);

	if (db_infos->compression)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_COMPRESSION_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_COMPRESSION_FLAG);

	if (db_infos->integrity_check)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_CHECK_CRC_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_CHECK_CRC_FLAG);

	if (db_infos->mac_address_lock)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_MAC_ADDRESS_LOCK_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_MAC_ADDRESS_LOCK_FLAG);

	if (db_infos->power_user_only)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_POWER_USER_ONLY_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_HDR_POWER_USER_ONLY_FLAG);

	if (db_infos->session_id_lock)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_SESSION_ID_LOCK_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_SESSION_ID_LOCK_FLAG);

	if (db_infos->read_only)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_READ_ONLY_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_READ_ONLY_FLAG);

	if (db_infos->indexing)
	{
	  SET_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_INDEXING_FLAG);
	}
	else REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_INDEXING_FLAG);

	if (db_infos->crash_flag == FALSE)
	{
	  REMOVE_ATTRIBUTE(db_handle->db_header.flags,LILLY_DB_CRASH_STATE_FLAG);
    }
...
boolean lilly_db_create_blob(const p_lilly_db_handle db_handle,const p_lilly_db_blob_name blob_id,const puchar data,const unsigned_int data_size,const uint32 operator_id)
{
  ...
  uchar blob_path[260 * 2];
	...
	if (db_handle->blob_path_len + blob_id->len > sizeof(blob_path) - 2)
	{
	  db_handle->error_code = IO_ERROR;
	  return FALSE;
	}
	...
	*(puint16)&blob_path[db_handle->blob_path_len + blob_id->len] = 0;

	avcore_memcpy(&blob_path,&db_handle->blob_folder_path,db_handle->blob_path_len);

	avcore_memcpy(&blob_path[db_handle->blob_path_len],&blob_id->name,blob_id->len);
...
p_lilly_db_blob_handle lilly_db_open_blob(const p_lilly_db_handle db_handle,const p_lilly_db_blob_name blob_id)
{
  p_lilly_db_blob_handle blob_handle;
	...
	if (db_handle->blob_path_len + blob_id->len > sizeof(blob_handle->blob_file_path) - 2)
	{
	  db_handle->error_code = IO_ERROR;
	  return NULL;
	}
	...
	avcore_memcpy(&blob_handle->blob_file_path[db_handle->blob_path_len],&blob_id->name,blob_id->len);
...
boolean lilly_db_read_blob(const p_lilly_db_blob_handle blob_handle,const puchar data,const unsigned_int data_size)
{
	...
	if (blob_handle->header.data_size != (uint64)data_size)
	{
	  blob_handle->error_code = API_ERROR;
	  return FALSE;
	}
...
boolean lilly_db_add_index(const p_lilly_db_handle db_handle,const uint32 index,const uint64 offset)
{
  unsigned_int table_index,item_index;
	 ...
	 if (!db_handle->index_blocks_params.index_blocks_count)
	 {
	   db_handle->error_code = LILLY_DB_FEATURE_UNAVAILABLE;
	   return FALSE;
	 }
	 else if (((int32)index < 0) || ((int64)offset <= 0))
	 {
	   db_handle->error_code = API_ERROR;
	   return FALSE;
	 }

	 table_index = (unsigned_int)index / sizeof(*db_handle->index_blocks_params.blocks);

	 if (table_index > db_handle->index_blocks_params.index_blocks_count)  // enlargement work by incrementing one block each time
	 {
	   db_handle->error_code = INDEX_OUT_OF_RANGE;
	   return FALSE;
	 }
	 else if (table_index == db_handle->index_blocks_params.index_blocks_count)
	 {
	   unsigned_int mem_size = (++db_handle->index_blocks_params.index_blocks_count) * sizeof(*db_handle->index_blocks_params.blocks);
	   p_lilly_db_index_block new_block;

	   new_block = (p_lilly_db_index_block)lilly_realloc(db_handle->index_blocks_params.blocks,mem_size - sizeof(*db_handle->index_blocks_params.blocks),mem_size);
	   ...
	   avcore_memset(&new_block[table_index],0,sizeof(*db_handle->index_blocks_params.blocks));
	 }

	 item_index = (unsigned_int)index - (table_index * sizeof(*db_handle->index_blocks_params.blocks));

	 db_handle->error_code = ERROR_SUCCESS;

	 db_handle->index_blocks_params.blocks[table_index].indexes[(unsigned_int)index] = offset;

	 return TRUE;
}

boolean lilly_db_del_index(const p_lilly_db_handle db_handle,const uint32 index)
{
  unsigned_int table_index,item_index;
	 ...
	 if (table_index >= db_handle->index_blocks_params.index_blocks_count)
	 {
	   db_handle->error_code = INDEX_OUT_OF_RANGE;
	   return NULL;
	 }
	 ...
	 db_handle->index_blocks_params.blocks[table_index].indexes[item_index] = 0;

     return TRUE;
}

opaque_ptr lilly_db_index_to_record(const p_lilly_db_handle db_handle,const unsigned_int record_size,const uint32 index)
{
  uint64 offset;
  unsigned_int table_index,item_index;
	 ...
	 else if (((signed_int)record_size <= 0) || ((int32)index < 0))
	 {
	   db_handle->error_code = API_ERROR;
	   return NULL;
	 }
	 ...
	 offset = db_handle->index_blocks_params.blocks[table_index].indexes[item_index];

	 if (!offset) return NULL;

	 return lilly_db_read_record(db_handle,record_size,offset);
}

uint64 lilly_db_index_to_record_offset(struct t_lilly_db_handle *db_handle,const uint32 index) // duplicate code oui, mais index to record_ptr doit être optimisée avec le moins de call overhead
{
  unsigned_int table_index,item_index;
	 ...
	 return db_handle->index_blocks_params.blocks[table_index].indexes[item_index];

}

boolean lilly_db_indexing_available(const p_lilly_db_handle db_handle)
{

	 return ((db_handle) && (db_handle->index_blocks_params.index_blocks_count));
}
